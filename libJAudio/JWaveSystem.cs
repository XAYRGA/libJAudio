...
namespace libJAudio
{
    ...
    public class JWaveGroup : JBaseObject
    {
        public string awFile;
        public JWaveInfo[] Waves;
        public Dictionary<int, JWaveInfo> WaveByID;
    }
    ...
    */
    ...
    internal class WaveID : JBaseObject // Helper class for  returning all data when reading each entry inside of the C_DF.
    {
        public short awid;
        public short waveid;
    }
    ...
    public class JWaveSystem : JBaseObject
    {
        public int id;
        public JWaveGroup[] Groups;
        public JWaveInfo[] LoadedWaves;
        public Dictionary<int, JWaveInfo> WaveTable;



        public const int WSYS = 0x57535953;
        public const int SCNE = 0x53434E45;
        public const int C_DF = 0x432D4446;

        private static int[] readWINF(BeBinaryReader binSteam, int Base)
        {
        ...
        private static int[] readWBCT(BeBinaryReader binSteam, int Base)
        {
        ...
        public static JWaveSystem readStream(BeBinaryReader binStream)
        {
            ...
            newWSYS.WaveTable = new Dictionary<int, JWaveInfo>();
            ...
            for (int i = 0; i < WSYSGroups.Length; i++)
            {
                ...
                var scenes = loadScene(binStream, Base); // Load the SCNE object
                {
                    ...
                    var IDMap = loadC_DFSection(binStream, Base); // load the C_DF, which gives us  an array of C_DF entries, containing awID and WaveID. 
        ...
        private static JWaveGroup readWaveGroup(BeBinaryReader binStream, int Base)
        {
            var newWG = new JWaveGroup(); // Create new Wavegroup object
            newWG.WaveByID = new Dictionary<int, JWaveInfo>(); // Initialize the dictionary (map) for the wave id's (used later)
            ...
            newWG.Waves = new JWaveInfo[waveCount]; // make an array with all of the wave counts, this is to store the waves we will read
            for (int i = 0; i < waveCount; i++)
            {
                binStream.BaseStream.Position = Base + WaveOffsets[i]; // Seek to the offset of each eave
                newWG.Waves[i] = JWaveInfo.loadWave(binStream, Base); // Then tell it to load. 
                newWG.Waves[i].wsysFile = newWG.awFile;
            }
            return newWG;
        }

        private static int[] loadScene(BeBinaryReader binStream, int Base)
        {
        ...
        private static WaveID[] loadC_DFSection(BeBinaryReader binStream, int Base)
        {
            ...
            var idmap = new WaveID[count]; // New array to store all the waveid's in
